...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800 font-weight-bold">Edit Mata Pelajaran</h1>
            ...
            <form action="{{ route('mapel.update', $mapel) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 form-group">
                        ...
                        <input type="text" name="kode_mapel" class="form-control @error('kode_mapel') is-invalid @enderror" value="{{ old('kode_mapel', $mapel->kode_mapel) }}" required>
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <select name="tingkat_id" class="form-control @error('tingkat_id') is-invalid @enderror" required>
                            ...
                            @foreach($tingkats as $t)
                                <option value="{{ $t->id }}" {{ (old('tingkat_id', $mapel->tingkat_id) == $t->id) ? 'selected' : '' }}>{{ $t->nama_tingkat }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <select name="kompetensi_keahlian_id" class="form-control @error('kompetensi_keahlian_id') is-invalid @enderror">
                            ...
                            @foreach($kompetensis as $k)
                                <option value="{{ $k->id }}" {{ (old('kompetensi_keahlian_id', $mapel->kompetensi_keahlian_id) == $k->id) ? 'selected' : '' }}>{{ $k->nama_kompetensi }}</option>
                            @endforeach
                        </select>
                ...
                <a href="{{ route('mapel.index') }}" class="btn btn-light font-weight-bold">Batal</a>
            </form>
        </div>
    </div>
</div>
