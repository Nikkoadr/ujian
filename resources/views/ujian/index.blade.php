...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    .question-card { animation: fadeIn .2s ease; }
    @keyframes fadeIn {
        from { opacity: 0; transform: translateY(6px); }
        to { opacity: 1; transform: translateY(0); }
    }

    .MathJax { font-size: 1.08em !important; }
    ...
    .exam-text p { margin-bottom: 1rem; }
    .exam-text strong { font-weight: 700; }
    .exam-text em { font-style: italic; }
    .exam-text img {
        display: block;
        margin: 1rem auto;
    ...
    .exam-text table, .exam-text th, .exam-text td {
    ...
    .exam-text th, .exam-text td {
        padding: 10px 12px;
        text-align: left;
    }
    .exam-text ul, .exam-text ol {
        margin-left: 1.5rem;
        margin-bottom: 1rem;
    }
    .exam-text ul { list-style-type: disc; }
    .exam-text ol { list-style-type: decimal; }
    .exam-text li { margin-bottom: .45rem; }
    .exam-text code {
        background: #f8fafc;
        color: #e11d48;
        padding: 2px 6px;
    ...
    .option-card { transition: all .2s ease; border: 2px solid transparent; }
    ...
    .nav-number { transition: all .2s ease; }
    .nav-number:hover { transform: scale(1.05); }
    .btn-transition { transition: all .2s ease; }
    .btn-transition:hover { transform: translateY(-1px); }
    .btn-transition:active { transform: scale(.98); }

    @media (max-width: 768px) {
        .font-size-small { font-size: .95rem; }
        .font-size-medium { font-size: 1rem; }
        .font-size-large { font-size: 1.15rem; }
        .exam-text { line-height: 1.8; }
        .exam-text table { display: block; overflow-x: auto; white-space: nowrap; }
        mjx-container { overflow-x: auto !important; overflow-y: hidden !important; }
    }
...
<body class="h-full overflow-hidden flex flex-col text-slate-700 bg-slate-100"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex gap-1 bg-white p-1 rounded-xl border border-slate-200">
                        <button @click="setFont('small')"
                                ...
                                class="w-8 h-8 rounded-lg font-bold text-[10px] transition-all duration-200">A</button>
                        <button @click="setFont('medium')"
                                :class="fontSize == 'medium' ? 'bg-sky-600 text-white shadow-sm' : 'bg-slate-50 text-slate-500'"
                                class="w-8 h-8 rounded-lg font-bold text-xs transition-all duration-200">A</button>
                        <button @click="setFont('large')"
                                :class="fontSize == 'large' ? 'bg-sky-600 text-white shadow-sm' : 'bg-slate-50 text-slate-500'"
                                class="w-8 h-8 rounded-lg font-bold text-sm transition-all duration-200">A</button>
                    </div>
                </div>
    ...
    <form id="form-selesai"
        action="{{ route('ujian.selesai', ['mapel' => $mapel->id]) }}"
        method="POST"
        class="hidden">
        @csrf
    </form>
